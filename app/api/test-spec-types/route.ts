...
export async function GET(request: NextRequest) {
  try {
    ...
    const productSpecsResult = await supabaseWithRetry(supabase, (client) =>
      client
        .from('product_specs')
    ...
    const { data: productSpecs, error: productSpecsError } = productSpecsResult as { data: any; error: any };
    ...
    const specsWithTypes = productSpecs.map((spec: any) => ({
      ...spec,
